function [Ybus, Yf, Yt, Ys] = Ybus_1(totalbus,totalbranch,all_busdata14,branch,bus)
...
%% constants
nb =  totalbus;     %size(bus, 1);    %% number of buses
nl =  totalbranch;   %size(branch, 1);%% number of lines
baseMVA=100;

%% define named indices into bus, branch matrices
[PQ, PV, REF, NONE, BUS_I, BUS_TYPE, PD, QD, GS, BS, BUS_AREA, VM, ...
    VA, BASE_KV, ZONE, VMAX, VMIN, LAM_P, LAM_Q, MU_VMAX, MU_VMIN] = idx_bus_1;
[F_BUS, T_BUS, BR_R, BR_X, BR_B, RATE_A, RATE_B, RATE_C, ...
    TAP, SHIFT, BR_STATUS, PF, QF, PT, QT, MU_SF, MU_ST, ...
    ANGMIN, ANGMAX, MU_ANGMIN, MU_ANGMAX] = idx_brch_1;

%% check that bus numbers are equal to indices to bus (one set of bus numbers)
if any(all_busdata14(:, BUS_I) ~= (1:nb)')
    error('makeYbus: buses must be numbered consecutively in bus matrix; use ext2int() to convert to internal ordering')
end
R=branch.SeriesResistance; 
X=branch.SeriesReactance;
...
%%      | It |   | Ytf  Ytt |   | Vt |
%%
stat = ones(nl,1);    %branch(:, BR_STATUS);                    %% ones at in-service branches
Ys = stat ./ (R + j * X);  %% series admittance
Bc = stat .* Bc;                           %% line charging susceptance
%tap = ones(nl, 1);                              %% default tap ratio = 1
%i = find(branch(:, TAP));                       %% indices of non-zero tap ratios
%tap(i) = branch(i, TAP);                        %% assign non-zero tap ratios
tap = branch.TapRatio;    %tap .* exp(1j*pi/180 * branch(:, SHIFT)); %% add phase shifters
Ytt = Ys + j*Bc/2;
...
G=bus.ShuntConductance;
B=bus.ShuntSusceptance;
Ysh = ( G+ j *B ) / baseMVA; %% vector of shunt admittances

%% build connection matrices
f =branch.Origin;       % branch(:, F_BUS);                           %% list of "from" buses
t =branch.Destination;  % branch(:, T_BUS);                           %% list of "to" buses
